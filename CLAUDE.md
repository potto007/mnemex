...
The WSL2 llama-server on localhost:8080 (gemma-4 GGUFs) serves the custom SFT'd model the prehend harness uses as its **solver/orchestrator** (e.g. `gemma-4-12b-it-sft-kb-v13-sft`). That model is the **student we evaluate; it is NOT a "teacher".** (The teacher / training-data-generation role lives entirely in rlm-trainer's `generate.py` - a separate upstream model like DeepSeek-V4-Flash or `gemma-4-12b-it-qat`. Never call the harness's served model a teacher.) When a run hitting this server times out (`ConnectTimeout`/`APITimeoutError`) but `curl`/another process to the "same" endpoint works and the server log shows `all slots are idle`:

**FIRST, check the actual socket destination: `ss -tnp | grep <pid>`.** A real case burned hours: the run was `SYN-SENT` to `172.19.144.1:1234` (the RETIRED Windows LM Studio endpoint), not `localhost:8080`, because `make_kb_realism.py` lacked `load_dotenv()` and `base_url` fell back to its hardcoded dead default. "Server idle + curl fast + run times out" = the run isn't talking to that server. Confirm the wire before tuning the server. (rlm-trainer data-gen scripts that hit this server MUST `load_dotenv()` like `generate.py` does, or be passed `TEACHER_BASE_URL` explicitly - that env var is rlm-trainer's, named for its data-gen role; it does NOT make the harness's served model a teacher.)
