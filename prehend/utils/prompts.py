...
2. A `llm_query(prompt, model=None, context=None, reduce=None)` function that makes a single LLM completion call (no REPL, no iteration). Fast and lightweight -- use this for SHORT text/extraction: pulling a fact, summarizing or answering over a SMALL chunk. Do NOT inline large chunks of context into the `prompt` string. **Easy path for LARGE text: pass it as `context=`** -- e.g. `llm_query("Which items does Dave own?", context=context)`. The harness then automatically splits the data into chunks, runs your prompt over each chunk in parallel, and combines the per-chunk results, so you do NOT have to slice by hand. Pass `reduce=` to give a distinct combine instruction (it defaults to your prompt), e.g. `llm_query("Extract every date Dave is mentioned", context=context, reduce="What is the earliest date?")`. A bare oversized `prompt` with no `context=` is still rejected with an instruction to chunk.
...
**CRITICAL - sub-calls do NOT see your `context`:** every sub-call (`llm_query`, `llm_query_batched`, `rlm_query`, `rlm_query_batched`) runs in a SEPARATE environment with NO access to your `context` variable. You MUST paste the relevant slice of context text directly into the prompt string you pass, e.g. `rlm_query(f"Which items does Dave own? Text:\\n{{context[:80000]}}")`. A bare instruction with no context slice (e.g. `rlm_query("find what Dave owns")`) gives the child nothing to read and it will answer "no information found". So DO pass context into sub-calls. The EASY path is `context=`: pass the big text via the `context=` argument and the harness chunks + map-reduces it for you automatically (no manual slicing needed). If you prefer to do it by hand, pass a SLICE (a chunk) into the prompt string, never the whole thing at once: keep each slice under ~{subcall_char_budget} characters and, when the context is larger, split it into several chunks and map-reduce them.
