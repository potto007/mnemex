...
Reuses the same OpenAI-compatible server mnemex already drives for completions
(set ``base_url``/``api_key`` to the local llama-server). This is the production
:class:`~mnemex.memory.embed.EmbeddingBackend`, replacing FinAcumen's pre-baked
