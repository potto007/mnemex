...
mnemex ``completion(prompt, root_prompt)`` interface -- in practice an
:class:`~mnemex.SRLM`. ``prompt`` is the context offloaded into the REPL;
...
from mnemex.memory.retrieve import (
    DEFAULT_K_MAX,
    DEFAULT_MIN_COSINE,
    retrieve,
)
